...
export function ProfilePage() {
  ...
  const filteredScrobbles = useMemo(() => {
    ...
    return currentScrobbles.filter(
      (s) =>
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      const artistMap = new Map<
        string,
        { count: number; artistId?: string; albumArtUrl?: string }
      >();
      ...
      try {
        ...
        const idImageMap = new Map<string, string>();
  ...
  const calculateMusicMatch = (
    ...
    const percentage =
      totalUnique > 0 ? Math.round((commonArtistNames.length / totalUnique) * 100) : 0;
  ...
  const getMatchLevelColor = (level: string) => {
    return (
      { low: '#ef4444', medium: '#f59e0b', high: '#22c55e', super: '#8b5cf6' }[level] || '#888'
    );
  };
  ...
  const currentTrack =
    isOwnProfile && currentlyPlaying?.item
      ? {
      ...
      <div className="profile-hero">
        ...
        <div className="profile-hero-content">
          <img
            src={
              user.avatarURL ||
              'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50" fill="%23555"/><text x="50" y="60" text-anchor="middle" fill="white" font-size="40">👤</text></svg>'
            }
            alt={user.name}
          ...
          <div className="section">
            <div className="section-header">
              ...
              <TextInput
                ...
                validationState={
                  searchQuery && filteredScrobbles.length === 0 ? 'invalid' : undefined
                }
              ...
              <>
                <div className="feed">
                  {paginatedScrobbles.map((scrobble) => {
                    ...
                    const isLikedByMe =
                      (isOwnProfile && tracksMode === 'liked') || likedScrobbleIds.has(scrobble.id);
                    return (
                      <ScrobbleCard
                        key={scrobble.id}
                        scrobble={scrobble}
                  ...
                  <div className="music-match-artists">
                    ...
                    {musicMatch.commonArtists.length > 3 && (
                      <span>
                        {' '}
                        {lang === 'ru'
                          ? `и ещё ${musicMatch.commonArtists.length - 3}`
                          : `and ${musicMatch.commonArtists.length - 3} more`}
                      </span>
                    )}
                  </div>
                </div>
        ...
        <Dialog.Header
          ...
          insertBefore={
            selectedArtist?.imageUrl ? (
              <img src={selectedArtist.imageUrl} alt="" className="artist-dialog-avatar" />
            ) : undefined
          }
        />
        <Dialog.Body>
                  ...
                  <div className="artist-dialog-tracks-list">
                    {artistTracks.map((track, idx) => (
                      <div key={`${track.id}-${idx}`} className="artist-dialog-track">
                        {track.albumArtURL && (
                          <img src={track.albumArtURL} alt="" className="artist-dialog-track-art" />
                        )}
